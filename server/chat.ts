...
import {
  MISCONCEPTION_SYSTEM_ADDENDUM,
  VALENCE_CONSISTENCY_ADDENDUM,
  MEDICAL_SAFETY_ADDENDUM,
  parseCounselorTurn,
  type CounselorTurnResult,
} from './misconception.js';

export type { CounselorTurnResult };
...
1. **動態情境破冰 (Dynamic Contextual Opening)**：
   ...
   - 語氣必須與情緒標籤同向：正向標籤用正向／中性好奇；負向標籤才同理難受。
   - 正確範例（正向）：「看到你選了『實習結束』，心裡是『開心、興奮』——再一天就結束了，聽起來你挺期待的，願意說說是什麼讓你這麼開心嗎？」
   - 正確範例（負向）：「看到你選了『分組報告』和『委屈』，今天是不是遇到什麼卡住的事了？」
   - 錯誤範例（禁止）：「聽起來除了興奮又快樂，『實習結束/離職』也讓你很不好受」
...
7. **客觀情緒調適引導 (Evidence-based Regulation)**：
   ...
   - 正向一致時，不要硬塞「調適／冷靜下來」技巧，除非使用者主動需要。

【回應結構參考 (請自然融合於對話中)】
- **接住 (Acknowledge)**：重述使用者的處境與事實。
- **同理 (Empathize)**：點出該處境下產生這種情緒的合理性（情緒用字須與價値同向）。
- **引導 (Guide)**：拋出一個符合 RULER 框架的溫和提問。

請一律使用繁體中文回覆。
` + VALENCE_CONSISTENCY_ADDENDUM + MISCONCEPTION_SYSTEM_ADDENDUM + MEDICAL_SAFETY_ADDENDUM;
...
function buildSystemInstruction(context?: ChatContext): string {
  ...
  if (context?.isOpening) {
    parts.push(
      `\n【本次任務 — 動態開場】使用者剛完成打卡，你尚未發言。請依「動態情境破冰」與「情緒價値一致性」規則，將上述情緒與事件自然融入第一句話，並以「一個」溫和探詢結尾。` +
        `\n若情緒標籤偏正向：肯定其正向感受，事件用中性描述，禁止「不好受／很難／受傷」等負向預設。` +
        `\n若情緒標籤偏負向：同理難受並邀請多說。` +
        `\n僅輸出開場白純文字本身（不要 JSON、不要自我介紹名字、不要說「這裡沒有對錯」、不要說「我在」、不要以第三人稱稱呼自己）。`,
    );
  }
  return parts.join('');
}
...
export async function generateChatReply(
  ...
  const raw = await dispatchToProvider(messages, systemInstruction, provider, options?.clientApiKey);
  return parseCounselorTurn(raw);
}
